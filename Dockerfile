...
#Making datacopy directory - cache dir
RUN mkdir /data-copy
...
#Syncing local files with backup 
COPY sync.sh /app/
RUN chmod +x /app/sync.sh

#Add a cron job to sync files evey 5 mins
RUN echo "*/5 * * * * bash /app/sync.sh" >> /etc/crontab
RUN echo "27 11 * * * bash /app/sync.sh" >> /etc/crontab
